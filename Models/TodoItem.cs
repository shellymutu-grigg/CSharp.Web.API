...
namespace CSharp.Web.API.Models
{
	public class TodoItem : IValidatableObject
	{
		...
		static Boolean IsOnlyLettersAndDigitsAndNotWhiteSpace(string toValidate)
		{
			return toValidate.All(char.IsLetterOrDigit) && !String.IsNullOrWhiteSpace(toValidate);
		}
		...
		public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			// Validate that the name of the todo item is not null, whitespace or empty and is only letters and digits
			if (!String.IsNullOrEmpty(Name))
			{
				if (IsOnlyLettersAndDigitsAndNotWhiteSpace(Name))
				{
					Console.WriteLine("Name Is Only Letters And Digits And Name is Not White Space " + IsOnlyLettersAndDigitsAndNotWhiteSpace(Name));
				}
			...
			//try
			//{
			// Validate that the name of the todo item is not null, whitespace or empty
			//if (Id is null)
			//{
			//	IsANumberAndNotWhiteSpace(Convert.ToString(Id));
			//	Console.WriteLine("IsOnlyLettersNotNullAndNotWhiteSpace");
			//}
			//else
			//{
			//	throw new ArgumentNullException(nameof(nullException));
			//}
